...
public class GridBuilding : MonoBehaviour {
    ...
    void controllerOptions(variableData data)
    {
        if (GameObject.FindObjectOfType<Recycle>().Money >= data.f)
        {
            ...
            if (temp != -1)
            {
                if (grid[temp].full == false)
                {
                    ...
                    {
                        BuildBlock();
                        GameObject.FindObjectOfType<Recycle>().Money -= (int)data.f;
                    }
                    if (data.i == 1)
                    {
                        BuildTurret();
            ...
            if (data.i == 2)
            {
                if (temp != -1)
                {
                    ...
                    {
                        DestroyBlock();
                        GameObject.FindObjectOfType<Recycle>().Money += 1;
                    }
                }
            }
        }
    ...
    void BuildBlock()
    {
        ...
        if (temp != -1)
        {
            if (grid[temp].full == false)
            {
                createObj(temp, obj[1], true, true, true, false);
    ...
    void BuildTurret()
    {
        ...
        if (temp != -1)
        {
            if (grid[temp].full == false)
            {
                createObj(temp, obj[0], false, false, false, true);
                GameObject.FindObjectOfType<checkWithinCamera>().testSections();
            }
        }
    }
	...
	bool checkBlockPlaceAvailable(int _index, bool testCase, bool solidTestCase, bool needsRotation, bool canPlaceOnGround)
    {
        checkSidesData temp = checkSides(_index);
	...
	void createObj(int _index, GameObject _obj, bool _isSolid, bool _breakable, bool canPlaceOnGround, bool needsRotation, bool force = false) {
    ...
    void fillHole()
    {
        ...
        if (temp != -1)
        {
            ...
            {
                createObj(sections[a].index, obj[currentObj], true, true, true, false);
            }
        }
    }
